...
function drawPlayers(players, x, y, w, h) {
    ...
    for(i in players) {
        ...
        render.fillStyle = "white";
        render.fillText(p.wins + "-" + p.losses + "-" + p.ties + " " + (p.score >= 0 ? "+" : "") + p.score.toFixed(2), loc + areaEach/2, y + 43, areaEach - 20);
...
function drawBoard(board, matches, loc) {
    ...
    if(board.matchID == null) {
        ...
        if(match.finished) {
            render.fillStyle = "black";
            render.roundRect(x + w/2 - 50, y+w/2 - 15, 100, 30, 5).fill();
            render.fillStyle = "white";
            if(match.winner == "tie") {
                render.fillText("tie", x + w/2 - 20, y + w/2 + 5);
            } else {
                render.fillText("won", x + w/2 + 10, y + w/2 + 5);
                render.fillStyle = getColorFromPlayerID(match.winner);
                render.roundRect(x + w/2 - 40, y + w/2 - 7, 40, 14, 3).fill();
            }
        }
    }
}

...
class GameController {
    ...
    val tournament = RRTournament(listOf("secret1", "secret2", "secret3", "secret4", "secret5", "secret6"), 6, { MNKBoard(5, 5, 4) })

    /**
     * Get the current board for a player to solve as a json object:
     * null|
     * [[Int, Int, ...], ...]
     * Each cell in the board is an integerr. A -1 indicates the cell is open, a 0 indicates it has the player's piece, a 1 indicates it has the opponents piece
     */
    ...
    fun getBoardToPlay(@RequestParam key: String): String {
        ...
        return "${board.board.contents.mapIndexed {x, _ -> board.board.contents[x].mapIndexed {y, _ ->
